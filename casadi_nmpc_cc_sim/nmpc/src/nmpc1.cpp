...
//定点控制初始化函数
NMPC1::NMPC1(int predict_step, float sample_time,Eigen::Matrix<float,6,1> desired_states)
{
    m_predict_step = predict_step;
    m_sample_time = sample_time;
    m_nmpc_stage = -1;
    m_control_command<<0,0;
    m_desired_states = desired_states;
    m_task = "StationKeeping";
    ...
    //设置求解器
    set_my_nmpc_solver_for_StationKeeping();
}

//轨迹跟踪初始化函数
NMPC1::NMPC1(int predict_step, float sample_time, std::vector<Eigen::Matrix<float,8,1>> trajectory_sequence)
{
    m_predict_step = predict_step;
    m_sample_time = sample_time;
    m_nmpc_stage = -1;
...
//创建定点控制求解器
void NMPC1::set_my_nmpc_solver_for_StationKeeping()
{
    //模型建立
    ...
    casadi::SX Xref = opt_para(casadi::Slice(6,12,1));
    for(int k=0;k<m_predict_step;k++)
    {
        casadi::SX states_err = X(casadi::Slice(),k)-Xref;
...
//创建轨迹跟踪求解器
void NMPC1::set_my_nmpc_solver_for_TrajectoryTracking()
{
    //模型建立
    casadi::SX x = casadi::SX::sym("x");
    casadi::SX y = casadi::SX::sym("y");
...
//根据初始化条件来判断调用哪个求解函数
void NMPC1::opti_solution(Eigen::Matrix<float, 6, 1> current_states)
{
    if(m_task == "StationKeeping")
    {
        opti_solution_for_StationKeeping(current_states);
    }
    if(m_task == "TrajectoryTracking")
    {
        opti_solution_for_TrajectoryTracking(current_states);
    }
}

void NMPC1::opti_solution_for_StationKeeping(Eigen::Matrix<float, 6, 1> current_states)
{
    //设置控制约束
    ...
    for(int j=0;j<6;j++)
    {
        parameters.push_back(m_desired_states[j]);
    }
...
//轨迹跟踪优化求解函数
void NMPC1::opti_solution_for_TrajectoryTracking(Eigen::Matrix<float,6,1> current_states)
{
    //设置控制约束
    std::vector<float> lbx;
    std::vector<float> ubx;
...
Eigen::Matrix<float,2,1> NMPC1::get_controls()        // casadi::SX c13 = 0;
        // casadi::SX c23 = 0;
{
...
std::vector<Eigen::Matrix<float, 6, 1>> NMPC1::get_predict_trajectory()
{
