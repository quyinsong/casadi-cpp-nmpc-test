...
public:
    NMPC1(int predict_step, float sample_time,Eigen::Matrix<float,6,1> desired_states);
    NMPC1(int predict_step, float sample_time, std::vector<Eigen::Matrix<float,8,1>> trajectory_sequence);
    ~NMPC1();
    //定义求解器
    void set_my_nmpc_solver_for_StationKeeping();
    void set_my_nmpc_solver_for_TrajectoryTracking();
    //优化求解
    void opti_solution(Eigen::Matrix<float, 6, 1> current_states);
    void opti_solution_for_StationKeeping(Eigen::Matrix<float,6,1> current_states);
